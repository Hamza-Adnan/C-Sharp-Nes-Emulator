    ...
    sealed partial class CPU_CLASS
    {
        ...
        private uint _Address()
        {
            ...
            switch (def.Mode)
            {
                ...
                case AbsoluteX:
                    uint addr = NextWord();
                    if (def.PageBoundary && (addr & 0xFF00) != ((addr + X) & 0xFF00)) CycleNUM += 1;
                    return addr + X;
                case AbsoluteY:
                    addr = NextWord();
                    if (def.PageBoundary && (addr & 0xFF00) != ((addr + Y) & 0xFF00)) CycleNUM += 1;
                    return addr + Y;
                case IndirectX:
