...
namespace NESDOTNET.Mappers
{
    ...
    class MMC3 : BaseMapper
    {
        ...
        private readonly Cartridge_CONTROLLER.VRAMMirroringMode[] _mirroringModes = { Vertical, Horizontal };
        ...
        public MMC3(EMU_MAIN emulator) : this(emulator, ChipType.MMC3)
        {

        }

        public MMC3(EMU_MAIN emulator, ChipType chipType) : base(emulator)
        {
            _type = chipType;
        ...
        public override void ProcessCycle(int scanline, int cycle)
        {
            if (_emulator.PPU.F.RenderingEnabled && cycle == 260 && (0 <= scanline && scanline < 240 || scanline == -1))
            {
                ...
                {
                    _irqCounter--;
                    if (_irqEnabled && _irqCounter == 0) _emulator.CPU.TriggerInterrupt(CPU_CLASS.InterruptType.IRQ);
                }
            }
        }
